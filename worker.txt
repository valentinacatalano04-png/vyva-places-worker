export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/journey") {
      ...
      // When the user explicitly chooses Metro, prefer changes between Tube
      // lines over long street-walking sections. TfL can route across the
      // whole Underground network and return the interchange legs itself.
      const metroOnly = safeModes === "tube";
      tfl.searchParams.set(
        "journeyPreference",
        metroOnly ? "LeastWalking" : "LeastTime"
      );
